...
import platform
import random
import webbrowser # FB Redirect এর জন্য যোগ করা হয়েছে

# Tool: TEAM-CZUCA-Dashboard-Ultimate
# Author: LEVIATHAN DRIFT 419
# Description: Intro Animation, 5-Step Loader, FB Redirect & Visual Preview
...
def show_connection_and_redirect():
    clear()
    print(f"\n{C}[*] ESTABLISHING CONNECTION TO SERVER...{RESET}")
    time.sleep(0.5)
    ...
    print(f"{G}[✔] Connection Successful! Resuming Tool...{RESET}")
    time.sleep(1.5)
...
def intro_animation():
    clear()
    cols = get_cols()
    print(f"\n{BK}[INIT] CONNECTION HOBE TEAM CZUCA...{RESET}")
    time.sleep(0.5)
...
"""
    print(logo)
    print(f"{BK}========================================================================{RESET}")
    print(f"{Y}[ TEAM CZUCA TERMINAL ]     {R}OWNER: LEVIATHAN DRIFT 419   {G}STATUS: ONLINE{RESET}")
    print(f"{BK}========================================================================{RESET}")
...
def main():
    ...
    except: name = "TEAM-CZUCA"
    if not name: name = "TEAM-CZUCA"
    ...
    # --- BASHRC INSTALL (100% ERROR-FREE OLD LOGIC WITH NEW BRANDING) ---
    bashrc_content = f"""
# --- TEAM CZUCA SYSTEM ---
clear
PS1='\\[\\033[1;36m\\]┌──(\\[\\033[1;31m\\]TEAM-CZUCA💀Termux\\[\\033[1;36m\\])-[\\[\\033[1;37m\\]\\w\\[\\033[1;36m\\]]\\n\\[\\033[1;36m\\]└─\\[\\033[1;33m\\]$\\[\\033[0m\\] '
COLS=$(tput cols)
C="\\033[1;36m"
R="\\033[1;31m"
W="\\033[1;37m"
...
printf "╗\\n"
printf "$C║ $R● $W TEAM CZUCA TERMINAL $R● $C USER: $Y{name.upper()}"
printf "\\n"
printf "$C╠"
for ((i=1; i<=COLS-2; i++)); do printf "═"; done
printf "╣\\n"
